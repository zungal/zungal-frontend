...
# ---------------------------------------------------------------------------- 
def mk_prc_df(tickers: List[str], prc_col: str='adj_close'):
...
# ---------------------------------------------------------------------------- 
def get_avg(df, col: str, year: int):
    ...
    df['Date'] = df.index
    records = df.loc[df['Date'].dt.year == year]
    return records[col].mean()
...
def get_ew_rets(df, tickers):
    ...
    df = df.fillna(0)
    ans = df[tickers]
    return (ans.iloc[:, 0] + ans.iloc[:, 1]) / 2
...
def get_ann_ret(ser, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    x = pd.dropna(series, axis=0)
    date = pd.date_range(start=start, end=end)
    return (series[date].sum())**(252 / x.shape[0]) - 1
